...
TargetWorkspaceName:
  ...
  DeleteArtifactsNotInTemplate:
    ...
    required: false
  managedIdentity:
    description: 'Use managed identity to generate the bearer token? (Takes precedence over federatedIdentity and clientSecret if present)'
    required: false
  federatedIdentity:
    description: 'Use federated identity to generate the bearer token? (Takes precedence over clientSecret if present)'
    required: false
